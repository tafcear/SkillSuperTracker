...
1. **项目命名**：显示名 **SkillSuperTracker**（2026-08-23 晚用户更名裁定，GitHub 仓库同步改名 tafcear/SkillSuperTracker，旧链接自动重定向）；**npm 包名与 CLI 保持小写 `skillsupertracker`**（npm 注册表禁止大写包名；skill-tracker 已被占用 1.0.1）
2. **GitHub 远端**：**public 开源仓库**（tafcear/SkillSuperTracker）；**License 待定**（用户将与作者讨论，定案前仓库不设 LICENSE 文件、README 标注「License 待定」）
